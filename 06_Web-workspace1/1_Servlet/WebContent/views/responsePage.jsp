<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%
	// 이 구문을 스크립틀릿이라고 해서 html 문서 내에 JAVA 코드를 기술할 수 있는 영역 (자바주석)
	// 현재 이 jsp 에서 필요로 하는 데이터들 => request 객체의 attribute 영역에 (key-value 세트로) 담겨있음
	// request.getAttribute("키값") : Object 타입의 밸류값 리턴
	// Object 타입은 모든 타입의 최상위 부모이기 때문에 "강제형변환" 해서 변수에 담으면 됨
	// => 주의할점 : 다형성에 의해 잘못 DownCasting 하면 ClassCastException 발생!!
...
<body>
	...
	<% } else { %>
		<% if(gender.equals("M")) { %>
			<span id="gender">남자</span> 입니다. <!-- case 2 -->
		<% } else { %>
			<span id="gender">여자</span> 입니다. <!-- case 3 -->
		<% } %>
	<% } %>
	<br>
